...
do $$ begin create type user_role as enum ('CUSTOMER', 'DRIVER', 'VENDOR', 'ADMIN'); exception when duplicate_object then null; end $$;
do $$ begin create type order_type as enum ('FOOD', 'SHOP', 'PARCEL'); exception when duplicate_object then null; end $$;
do $$ begin create type order_status as enum ('PLACED', 'VENDOR_ACCEPTED', 'VENDOR_REJECTED', 'DRIVER_ASSIGNED', 'ACCEPTED', 'PREPARING', 'READY_FOR_PICKUP', 'PICKED_UP', 'ON_THE_WAY', 'DELIVERED', 'CANCELLED'); exception when duplicate_object then null; end $$;
do $$ begin create type payment_type as enum ('COD'); exception when duplicate_object then null; end $$;
...
do $$ begin create type delivery_status as enum ('PENDING_ASSIGNMENT', 'ASSIGNED', 'ACCEPTED', 'PICKED_UP', 'ON_THE_WAY', 'DELIVERED'); exception when duplicate_object then null; end $$;
...
create index if not exists idx_tracking_delivery_time on delivery_tracking(delivery_id, created_at desc);
